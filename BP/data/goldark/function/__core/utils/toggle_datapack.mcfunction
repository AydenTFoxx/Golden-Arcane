...
execute if data storage goldark:settings { benchmark: true } as @a run function goldark:__core/utils/log_message { message: "Datapack enabled; Spells, effects and other features will resume their usual behavior.", color: "white" }
execute if data storage goldark:settings { benchmark: true } run return run data remove storage goldark:settings benchmark


## DISABLE

execute as @a run function goldark:__core/utils/log_message { message: "Datapack disabled; All behaviors are disabled and most features cannot be interacted with.", color: "white" }
