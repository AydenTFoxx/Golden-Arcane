...
execute as @n[type=item, tag=goldark.item_drop, distance=..1] if data entity @s Item.components."minecraft:custom_data".goldark.StoredSpell.data run data modify entity @s Item.components.minecraft:lore set value [ { text: "Place on the ground to release the stored spell.", color: "white", italic: false } ]
